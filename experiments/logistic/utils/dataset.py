...
class Dataset(Dataset):
    def __init__(self, directory):
        ...
        mean = np.mean(self.lh_fmri, axis=0)
        maxi = np.max(self.lh_fmri, axis=0)
        mini = np.min(self.lh_fmri, axis=0)

        self.lh_fmri = 2*(self.lh_fmri - mini)/(maxi - mini) - 1

        print('\nLH training fMRI data shape:')
        print(self.lh_fmri.shape)
        print('(Training stimulus images × LH vertices)')
        plt.hist(self.lh_fmri.flatten(), bins=100)
