...
class Dataset(Dataset):
    def __init__(self, directory):
        ...
        left_mean = np.mean(self.lh_fmri, axis=0)
        left_maxi = np.max(self.lh_fmri, axis=0)
        left_mini = np.min(self.lh_fmri, axis=0)

        right_mean = np.mean(self.rh_fmri, axis=0)
        right_maxi = np.max(self.rh_fmri, axis=0)
        right_mini = np.min(self.rh_fmri, axis=0)

        self.lh_fmri = 2*(self.lh_fmri - left_mini)/(left_maxi - left_mini) - 1
        self.rh_fmri = 2*(self.rh_fmri - right_mini)/(right_maxi - right_mini) - 1
